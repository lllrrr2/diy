...
function pollLog(e) {
	return Promise.all([
		fs.exec_direct('/usr/libexec/qbittorrent-call', [ 'tail' ]).then(function(res) {
			return res.trim().split(/\n/).reverse().join('\n')
		}),
		fs.exec_direct('/sbin/logread', [ '-e', 'qbittorrent' ]).then(function(res) {
...
return view.extend({
	render: function() {
		...
		return E([
			E('style', [ css ]),
			E('div', {'class': 'cbi-map'}, [
				E('h2', {'name': 'content'}, '%s - %s'.format(_('qBittorrent'), _('Log Data'))),
