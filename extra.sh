...
package_name="@types/node axios canvas crypto-js date-fns dotenv fs jsdom png-js require ts-md5 ts-node tslib typescript"
requirement_name="cryptography~=3.2.1 json5 requests rsa"
## 3. Ninja 是否需要启动和更新设置
Ninja=" " ##up为更新，on为启动，down为不运行
...
# 🌱拉取仓库
CR1(){
    ql repo https://github.com/shufflewzc/faker2.git "jd_|jx_|gua_|jddj_|getJDCookie" "activity|backUp|Coupon|update" "^jd[^_]|USER|utils|function|^JS|^TS|^JDJRValidator_Pure|^ZooFaker|^sign|ql|sendNotify"
CR2(){
...
OR3(){
    ql repo https://github.com/smiek2221/scripts.git "gua_" "" "^MovementFaker|^JDJRValidator|^ZooFaker|^sign"
}
OR5(){
